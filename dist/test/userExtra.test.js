...
describe('UserController - extra', () => {
    ...
    it('delete retorna 500 em exceção', async () => {
        User.destroy.mockRejectedValueOnce(new Error('db error'));
        const res = makeRes();
        await ctrl.delete(makeReq({ params: { id: '1' } }), res);
    ...
    it('register cria usuário com sucesso', async () => {
        ...
        User.create.mockResolvedValueOnce({ id: 10, nome: 'Novo', email: 'novo@test.com' });
        const res = makeRes();
    ...
    it('updateMe retorna 400 quando nome muito curto', async () => {
        User.findByPk.mockResolvedValueOnce({ id: 1, cpf: '52998224725', update: vi.fn() });
        const res = makeRes();
