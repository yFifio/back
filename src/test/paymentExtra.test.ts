...
type TestResponse = Response & { status: Mock; json: Mock; sendStatus: Mock };
...
describe('PaymentNotificationController - handleNotification extra', () => {
  ...
  it('ignores already-processed webhook', async () => {
    (Order.findByPk as Mock).mockResolvedValue({ id: 100, status: 'pending', save: vi.fn() });
    (PaymentWebhook.findOne as Mock).mockResolvedValue({ processed: true });
  ...
  it('extracts payment id from resource object', async () => {
    (Order.findByPk as Mock).mockResolvedValue({ id: 5, status: 'pending', save: vi.fn() });
    (PaymentWebhook.findOne as Mock).mockResolvedValue({ processed: true });
  ...
  it('extracts payment id from resource string', async () => {
    (Order.findByPk as Mock).mockResolvedValue({ id: 6, status: 'pending', save: vi.fn() });
    (PaymentWebhook.findOne as Mock).mockResolvedValue({ processed: true });
  ...
  it('extracts payment id from action field', async () => {
    (PaymentWebhook.findOne as Mock).mockResolvedValue({ processed: true });
    (Order.findByPk as Mock).mockResolvedValue({ id: 7, status: 'pending', save: vi.fn() });
  ...
  it('catches and acknowledges when processing flow throws', async () => {
    (Order.findByPk as Mock).mockResolvedValue({ id: 80, status: 'pending', save: vi.fn().mockResolvedValue(undefined) });
    (PaymentWebhook.findOne as Mock).mockResolvedValue(null);
    (PaymentWebhook.create as Mock).mockRejectedValue(new Error('db write error'));
...
describe('PaymentNotificationController - getPaymentStatus extra', () => {
  ...
  it('retorna status sem payment_id', async () => {
    ...
    (Order.findByPk as Mock).mockResolvedValueOnce(order);
    (PaymentWebhook.findAll as Mock).mockResolvedValueOnce([]);
    const res = makeRes();
