...
describe('CategoryController', () => {
  ...
  it('list retorna categorias quando db conectado', async () => {
    (Category.findAndCountAll as Mock).mockResolvedValueOnce({ count: 2, rows: [{ id: 1, name: 'A' }, { id: 2, name: 'B' }] });
    const res = makeRes();
  ...
  it('list retorna 500 em exceção', async () => {
    (Category.findAndCountAll as Mock).mockRejectedValueOnce(new Error('db error'));
    const res = makeRes();
    await ctrl.list(makeReq({ query: {} }), res);
  ...
  it('getById retorna categoria existente', async () => {
    (Category.findByPk as Mock).mockResolvedValueOnce({ id: 1, name: 'Livros' });
    const res = makeRes();
    await ctrl.getById(makeReq({ params: { id: '1' } }), res);
  ...
  it('getById retorna 404 quando não existe', async () => {
    (Category.findByPk as Mock).mockResolvedValueOnce(null);
    const res = makeRes();
  ...
  it('getById retorna 500 em exceção', async () => {
    (Category.findByPk as Mock).mockRejectedValueOnce(new Error('db fail'));
    const res = makeRes();
    await ctrl.getById(makeReq({ params: { id: '1' } }), res);
    expect(res.status).toHaveBeenCalledWith(500);
  });
  ...
  it('create cria categoria com sucesso', async () => {
    (Category.create as Mock).mockResolvedValueOnce({ id: 5, name: 'Nova' });
    const res = makeRes();
  ...
  it('create retorna 500 em exceção', async () => {
    (Category.create as Mock).mockRejectedValueOnce(new Error('fail'));
    const res = makeRes();
  ...
  it('update retorna 500 em exceção', async () => {
    (Category.findByPk as Mock).mockRejectedValueOnce(new Error('fail'));
    const res = makeRes();
