...
describe('ProductController', () => {
  ...
  it('delete exclui produto com sucesso', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ success: true, action: 'deleted' });
  });
  ...
  it('delete desativa produto quando há vínculo em pedidos', async () => {
    const fkError = Object.assign(new Error('Cannot delete or update a parent row'), {
      name: 'SequelizeForeignKeyConstraintError',
      parent: { code: 'ER_ROW_IS_REFERENCED_2' },
    });
    (Product.destroy as Mock).mockRejectedValueOnce(fkError);
    (Product.update as Mock).mockResolvedValueOnce([1]);
    ...
    expect(res.json).toHaveBeenCalledWith({ success: true, action: 'deactivated' });
  });
});
